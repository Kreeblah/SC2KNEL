...
RESTARTDUETOFAILURE:
        ...
        Dim RequestCode As String = gsseckey(System.Text.Encoding.ASCII.GetBytes(spl(spl.Length - 1)), 2, "SC2KNE")

        Dim packet() As Byte = System.Text.Encoding.ASCII.GetBytes("\basic\gamename\sc2kne\enctype\2\location\0\validate\" & RequestCode & "\final\list\\gamename\sc2kne\final\")
        ...
        End While
        tcp.Close()
        Dim ip As String = enctype2_decoder(buff, idn, "SC2KNE")
#If 0 Then
...
Retry:
    ...
    Public Sub AddToList(ByVal packet As String, ByVal state As String, ByVal ipport As Net.IPEndPoint)
        ...
        If packet.StartsWith("\gamename\") Then
            ...
            serverstatus.TryGetValue("gamever", str(3))
            ...
            str(4) = ipport.Address.ToString & ":" & hostport
            str(5) = ipport.Port.ToString
            Dim itm As New ListViewItem(str)
            If Not ServerListContainer.Items.Contains(itm) Then
                ServerListContainer.Items.Add(itm)
                If passworded = "1" Then PasswordedServersList.Add(str(4))
                Me.Invalidate()
            End If
        End If
    End Sub
    Private Sub JoinServer()
        ...
        Dim ip As String = selecteditem.SubItems(4).Text
        Dim splip = ip.Split(":") 'split the ip:port
        Dim iponly As String = splip(0)
        ...
        If EnteredServerPassword Then
            ...
            Me.Close()
        End If
        'todo: rewrite all code below in this function, it's shit
        If IO.File.Exists(CurrentDir & "\2KCLIENT.ini") Then
            Dim contents As String = IO.File.ReadAllText(CurrentDir & "\2KCLIENT.ini")
            If Not contents.Contains("Server IP") Then
                CreateNewSC2KClientINI(iponly)
            Else
                Dim spl = contents.Replace(vbCrLf, vbLf).Split(vbLf)
                Dim found As Boolean = False
                Dim newcontents As String = String.Empty
                For Each lin In spl
                    If lin.Contains("Server IP") Then
                        found = True
                        newcontents &= "Server IP=" & StrToHex(iponly) & vbCrLf
                    Else
                        lin = lin.Replace(vbLf, "")
                        newcontents &= lin & vbCrLf
                    End If
                Next
                While newcontents.EndsWith(vbCrLf)
                    newcontents = newcontents.Remove(newcontents.Length - 2, 2)
                End While
                While newcontents.EndsWith(vbLf)
                    newcontents = newcontents.Remove(newcontents.Length - 1, 1)
                End While
                newcontents &= vbCrLf
                If Not found Then
                    CreateNewSC2KClientINI(iponly)
                Else
                    UpdateSC2KClientINI(System.Text.Encoding.ASCII.GetBytes(newcontents))
                End If
            End If
        Else
            CreateNewSC2KClientINI(iponly)
    ...
    Private Sub LaunchGame()
        Dim sc2kneci As New ProcessStartInfo
        sc2kneci.WorkingDirectory = CurrentDir
        sc2kneci.FileName = CurrentDir & "\2KCLIENT.exe"
        sc2kneci.UseShellExecute = False
        Dim sc2knes As New Process
        sc2knes.StartInfo = sc2kneci
        sc2knes.Start()
    End Sub
    Private Sub UpdateSC2KClientINI(ByVal contents As Byte())
        'todo: fix this ugly shit
        IO.File.Delete(CurrentDir & "\2KCLIENT.ini")
        IO.File.WriteAllBytes(CurrentDir & "\2KCLIENT.ini", contents)
    End Sub
    Private Sub CreateNewSC2KClientINI(ByVal ip As String)
        IO.File.Delete(CurrentDir & "\2KCLIENT.ini")
        Dim fs As New IO.StreamWriter(CurrentDir & "\2KCLIENT.ini", IO.FileMode.Create)
        fs.WriteLine("[Settings]")
        fs.WriteLine("Player Name=00")
        fs.WriteLine("Server IP=" & StrToHex(ip))
        fs.WriteLine("Music=00000001")
        fs.WriteLine("Sound=00000001")
        fs.Close()
    End Sub
